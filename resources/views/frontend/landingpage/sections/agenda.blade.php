<section id="agenda" class="py-24 bg-white">
  ...
  @php
    $agendaSection = \App\Models\AgendaSection::singleton();
    $agendas       = \App\Models\Agenda::where('status', 'publish')->orderBy('date', 'desc')->take(4)->get();
  @endphp
  ...
  <div class="max-w-7xl mx-auto px-6 relative z-10">
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12 reveal">
      <div>
        <h2 class="text-3xl md:text-4xl font-extrabold text-gray-900 leading-tight">
          {!! nl2br(e($agendaSection->title)) !!}
        </h2>
        <p class="text-gray-500 mt-3 text-sm max-w-lg">
          {{ $agendaSection->description }}
        </p>
      </div>
      <!-- <a href="{{ url('/#agenda') }}" class="btn-raise shrink-0 inline-flex items-center gap-2 text-blue-700 hover:text-white bg-transparent hover:bg-blue-700 border border-blue-200 hover:border-blue-700 rounded-full px-6 py-3 text-sm font-semibold transition">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      @foreach($agendas as $i => $a)
        <a href="{{ route('landing.agenda.detail', $a->slug) }}"
