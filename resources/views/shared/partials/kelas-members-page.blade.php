<style>
    #membersPage { transform: translateX(100%); transition: transform .28s cubic-bezier(.4,0,.2,1); }
    #membersPage.open { transform: translateX(0); }
</style>
...
<div id="membersPage"
    ...
    <div class="bg-white flex items-center gap-3 px-4 py-3 shrink-0"
        ...
        <div class="flex-1 min-w-0">
            <h2 class="font-bold text-slate-800 text-sm leading-tight">Anggota Kelas</h2>
            <p class="text-[11px] text-slate-400">
                <span id="membersPageCount">—</span> siswa/i bergabung
            </p>
        </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar">
        <div id="membersBody" class="px-0 py-0">
            <p class="text-sm text-slate-400 text-center py-6 px-4">Memuat...</p>
        </div>
    </div>

    {{-- Footer: Keluar dari Grup Agenda (agenda classrooms, siswa only) --}}
    @if($authUser->role !== 'guru' && ($isAgendaKelas ?? false))
    <div class="shrink-0 bg-white px-4 py-3" style="border-top:1.5px solid #e2e8f0">
        <button id="btnLeaveKelas"
                data-leave-url="{{ route('kelas.leave', $kelas->id) }}"
                data-redirect-url="{{ route('siswa.chat') }}"
            ...
            Keluar dari Grup Agenda
        </button>
    </div>

    {{-- Leave confirm bottom sheet --}}
    <div id="leaveConfirmSheet" class="fixed inset-0 z-[10001] hidden">
        <div id="leaveConfirmBackdrop" class="absolute inset-0 bg-black/40"></div>
        <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl px-5 py-5">
            ...
            <h3 class="font-bold text-slate-800 text-base mb-1">Keluar dari Grup Agenda?</h3>
            <p class="text-sm text-slate-500 mb-5">Kamu dapat bergabung kembali via link undangan di halaman agenda.</p>
            <button id="leaveConfirmYes"
                    class="w-full py-2.5 rounded-xl text-sm font-semibold text-white mb-2.5"
                    style="background:#ef4444">Ya, Keluar</button>
            <button id="leaveConfirmNo"
    ...
    <script>
    (function () {
        const btn       = document.getElementById('btnLeaveKelas');
        const sheet     = document.getElementById('leaveConfirmSheet');
        const backdrop  = document.getElementById('leaveConfirmBackdrop');
        const yesBtn    = document.getElementById('leaveConfirmYes');
        const noBtn     = document.getElementById('leaveConfirmNo');
        if (!btn || !sheet) return;
        ...
        yesBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                closeSheet();
                // Close members panel
                const mp = document.getElementById('membersPage');
                if (mp) mp.classList.remove('open');
                // Switch to read-only mode (hide input, show read-only bar)
                if (typeof window.enterReadOnlyMode === 'function') {
                    window.enterReadOnlyMode();
                }
                window.showFlashModal('success', 'Kamu sudah keluar dari kelas ini.');
            } catch (_) {
                closeSheet();
