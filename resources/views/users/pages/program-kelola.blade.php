...
@section('title', 'Kelola Program dan Kegiatan — ' . config('app.name', 'Konseling'))
...
<div class="sticky top-0 z-40"
    ...
    <div class="px-4 py-3 flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
            <h1 class="font-extrabold text-white text-base leading-tight">Kelola Program dan Kegiatan</h1>
            <p class="text-xs text-blue-200 leading-none mt-0.5">Program dan kegiatan yang Anda tambahkan</p>
        </div>
    </div>
</div>
...
<div class="px-4 md:px-8 py-5 pb-8 max-w-2xl md:max-w-4xl mx-auto">
    ...
    <div class="relative flex gap-3 bg-white border border-slate-100 rounded-2xl p-3.5 shadow-sm mb-3">
        ...
        <div class="flex-1 min-w-0 pr-8">
            ...
            <p class="text-sm font-bold text-slate-800 leading-snug line-clamp-2">{{ $programItem->title }}</p>
            <p class="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1">
                <i class="fa-regular fa-calendar text-[9px]"></i>
                {{ $programItem->date ? $programItem->date->format('d M Y') : '—' }}
            </p>
            @if($programItem->description)
            <p class="text-[11px] text-slate-500 mt-1 line-clamp-2 leading-relaxed">{{ $programItem->description }}</p>
            @endif
            <div class="flex items-center gap-1.5 mt-1.5 flex-wrap">
                @if($programItem->info_link)
        ...
        <div data-drop class="absolute top-3 right-3">
            ...
            <div data-drop-menu class="hidden fixed z-[9999] w-48 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
                {{-- Edit --}}
                <a href="{{ route('bk.program.edit', $programItem->slug) }}"
                ...
                <x-drop-action.delete
                    ...
                    :action="route('bk.program.destroy', $programItem->slug)" />
            </div>
        </div>
    </div>
    @empty
    <div class="min-h-[calc(100vh-120px)] flex flex-col items-center justify-center gap-3 text-center">
        ...
        <p class="text-sm text-slate-400 font-medium">Belum ada program dan kegiatan yang Anda tambahkan.</p>
    </div>
    @endforelse

    {{-- Pagination --}}
    @if($programs->hasPages())
        <div class="mt-4">{{ $programs->links('components.pagination.default') }}</div>
    @endif

</div>

{{-- FAB --}}
<a href="{{ route('bk.program.create') }}"
