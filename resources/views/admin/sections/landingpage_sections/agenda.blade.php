...
<head>
  ...
  <title>Agenda — {{ config('app.name') }}</title>
...
<body class="h-screen overflow-hidden bg-gray-50 text-slate-800">
    ...
    <header class="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-200">
      <div class="px-6 py-4">
        <div class="max-w-7xl mx-auto flex items-center justify-between gap-4">
          <div>
            <h1 class="text-lg md:text-xl font-semibold text-slate-900">Agenda</h1>
          </div>
    ...
    <div class="flex-1 overflow-y-auto bg-gray-50 p-3 sm:p-8"
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 hover:shadow-lg transition-all duration-300">
        ...
        <div class="px-8 py-5 border-b border-gray-200">
          ...
          <p class="text-sm text-gray-500 mt-1">Judul dan deskripsi header section Agenda di halaman publik</p>
        </div>
        ...
        <div class="p-8">
          ...
          <form x-show="editing" style="display:none;"
            id="sectionForm"
            method="POST" action="{{ route('admin.landing.agendaUpdate') }}"
            x-ref="sectionForm">
            @csrf @method('PUT')
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 hover:shadow-lg transition-all duration-300">
        ...
        <div class="px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 class="text-lg font-bold text-gray-900 uppercase tracking-wide">Data Agenda</h3>
            <p class="text-sm text-gray-500 mt-1">Daftar kegiatan &amp; program yang tampil di section Agenda ({{ $agendas->count() }} item)</p>
          </div>
          <button id="agendaOpenCreate" type="button"
        ...
        <div class="overflow-x-auto overflow-y-visible">
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                  ...
                  <div data-drop class="relative inline-block">
                    ...
                    <div data-drop-menu class="hidden fixed z-[9999] w-44 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
                      ...
                      <x-drop-action.toggle-publish
                        ...
                        :toggle-url="route('admin.landing.agendaToggle', $ag)" />
                      <x-drop-action.delete
                        :title="$ag->title"
                        :action="route('admin.landing.agendaDestroy', $ag)" />
                    </div>
                  </div>
                </td>
              </tr>
              @empty
              <tr>
                <td colspan="7" class="px-6 py-8 text-center text-gray-400">Belum ada data agenda.</td>
              </tr>
              @endforelse
            </tbody>
          </table>
  ...
  {{-- CRUD Modal --}}
  <div id="agendaCrudModal" class="fixed inset-0 z-[9999] hidden">
    <div class="absolute inset-0 bg-black/40" id="agendaModalBackdrop"></div>
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-xl max-h-[90vh] flex flex-col">
        <div class="px-6 py-4 border-b border-slate-200 shrink-0">
          <div class="text-sm text-slate-500">Data Agenda</div>
          <div id="agendaCrudTitle" class="text-lg font-semibold text-slate-900">Tambah Agenda</div>
        </div>
        <form id="agendaCrudForm" method="POST" action="{{ route('admin.landing.agendaStore') }}" enctype="multipart/form-data" class="p-6 space-y-4 overflow-y-auto flex-1">
              @csrf
              <input type="hidden" name="_method" id="agendaCrudMethod" value="POST">
              ...
              <div>
                ...
                <input id="agendaInputTitle" name="title" type="text" required maxlength="50"
              ...
              <div>
                ...
                <div class="flex items-start gap-3">
                  <div id="agendaImgPreviewBox" class="w-20 h-16 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center shrink-0">
                    <img id="agendaImgPreviewEl" src="" class="w-full h-full object-cover hidden" alt="Preview">
                    <i id="agendaImgPreviewIcon" class="fa-solid fa-image text-gray-300 text-xl"></i>
                  </div>
                  <div class="flex-1">
                    <input id="agendaInputImg" name="img" type="file" accept="image/*"
              ...
              <div>
                ...
                <div class="flex items-start gap-3">
                  <div id="agendaImgD1Box" class="w-20 h-16 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center shrink-0">
                    <img id="agendaImgD1El" src="" class="w-full h-full object-cover hidden" alt="Preview">
                    <i id="agendaImgD1Icon" class="fa-solid fa-image text-gray-300 text-xl"></i>
                  </div>
                  <div class="flex-1">
                    <input id="agendaInputImgD1" name="img_detail_1" type="file" accept="image/*"
                      class="w-full rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-primary/10 file:text-primary">
                    <p class="text-xs text-gray-400 mt-1" id="agendaImgD1Note">Ditampilkan di halaman detail (slide 1).</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="flex items-start gap-3">
                  <div id="agendaImgD2Box" class="w-20 h-16 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center shrink-0">
                    <img id="agendaImgD2El" src="" class="w-full h-full object-cover hidden" alt="Preview">
                    <i id="agendaImgD2Icon" class="fa-solid fa-image text-gray-300 text-xl"></i>
                  </div>
                  <div class="flex-1">
                    <input id="agendaInputImgD2" name="img_detail_2" type="file" accept="image/*"
                      class="w-full rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-primary/10 file:text-primary">
                    <p class="text-xs text-gray-400 mt-1" id="agendaImgD2Note">Ditampilkan di halaman detail (slide 2).</p>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input id="agendaInputCategory" name="category" type="text" required maxlength="60"
                ...
                <div>
                  ...
                  <input id="agendaInputDate" name="date" type="date" required
                    class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input id="agendaInputGuru" name="guru_pembimbing" type="text" maxlength="100"
                ...
                <div>
                  ...
                  <select id="agendaInputStatus" name="status" required
              ...
              <div>
                ...
                <textarea id="agendaInputDesc" name="description" rows="4" maxlength="2500"
              ...
              <div>
                ...
                <input id="agendaInputInfoLink" name="info_link" type="url" maxlength="500"
                  placeholder="https://..."
                  class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
              </div>

              {{-- Buat Grup Otomatis --}}
              <div id="agendaAutoGroupWrap" class="rounded-xl border border-blue-100 bg-blue-50 p-4">
                <label class="flex items-start gap-3 cursor-pointer select-none">
                  <input id="agendaInputAutoGroup" name="auto_group" type="checkbox" value="1"
                    class="mt-0.5 w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary accent-primary">
                  <div>
                ...
                {{-- Existing classroom badge (shown on edit when already linked) --}}
                <div id="agendaClassroomBadge" class="hidden mt-3 flex items-center gap-2 text-xs text-blue-700 bg-white border border-blue-200 rounded-lg px-3 py-2">
                  <i class="fa-solid fa-link-simple text-blue-400 shrink-0"></i>
                  <span>Terhubung ke grup: <strong id="agendaClassroomBadgeId"></strong></span>
                </div>
              </div>

          <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
            <button type="button" id="agendaCrudCancel"
  ...
  <div id="agendaDeleteModal" class="fixed inset-0 z-[9999] hidden">
    <div class="absolute inset-0 bg-black/40" id="agendaDeleteBackdrop"></div>
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-sm bg-white rounded-2xl border border-slate-200 shadow-xl p-6 text-center">
        ...
        <p class="text-sm text-slate-500 mb-5">
          "<span id="agendaDeleteName" class="font-medium text-slate-700"></span>" akan dihapus permanen.
        </p>
        <div class="flex gap-3">
          <button type="button" id="agendaDeleteCancel"
          ...
          <form id="agendaDeleteForm" method="POST" action="{{ route('admin.landing.agendaDestroy', 0) }}" class="flex-1">
            @csrf @method('DELETE')
            <button type="submit"
  ...
  // ── Agenda Items CRUD ───────────────────────────────────────────
  (function () {
    const storeUrl  = @json(route('admin.landing.agendaStore'));
    const updateTpl = @json(route('admin.landing.agendaItemUpdate', ['agenda' => '____ID____']));

    const crudModal      = document.getElementById('agendaCrudModal');
    const crudForm       = document.getElementById('agendaCrudForm');
    const crudTitle      = document.getElementById('agendaCrudTitle');
    const crudMethod     = document.getElementById('agendaCrudMethod');
    const inputImg       = document.getElementById('agendaInputImg');
    const imgPreviewEl   = document.getElementById('agendaImgPreviewEl');
    const imgPreviewIcon = document.getElementById('agendaImgPreviewIcon');
    const inputImgD1     = document.getElementById('agendaInputImgD1');
    const imgD1El        = document.getElementById('agendaImgD1El');
    const imgD1Icon      = document.getElementById('agendaImgD1Icon');
    const imgD1Note      = document.getElementById('agendaImgD1Note');
    const inputImgD2     = document.getElementById('agendaInputImgD2');
    const imgD2El        = document.getElementById('agendaImgD2El');
    const imgD2Icon      = document.getElementById('agendaImgD2Icon');
    const imgD2Note      = document.getElementById('agendaImgD2Note');
    const inputCategory  = document.getElementById('agendaInputCategory');
    const inputDate      = document.getElementById('agendaInputDate');
    const inputGuru      = document.getElementById('agendaInputGuru');
    const inputStatus    = document.getElementById('agendaInputStatus');
    const inputTitle     = document.getElementById('agendaInputTitle');
    const inputDesc      = document.getElementById('agendaInputDesc');
    const inputInfoLink  = document.getElementById('agendaInputInfoLink');
    const inputAutoGroup = document.getElementById('agendaInputAutoGroup');
    const autoGroupWrap  = document.getElementById('agendaAutoGroupWrap');
    const classroomBadge = document.getElementById('agendaClassroomBadge');
    const classroomBadgeId = document.getElementById('agendaClassroomBadgeId');
    const deleteModal    = document.getElementById('agendaDeleteModal');
    const deleteForm     = document.getElementById('agendaDeleteForm');
    const deleteName     = document.getElementById('agendaDeleteName');
    ...
    function openCreate() {
      crudTitle.textContent = 'Tambah Agenda';
      ...
      if (inputAutoGroup) { inputAutoGroup.checked = false; inputAutoGroup.disabled = false; }
      if (autoGroupWrap)  autoGroupWrap.classList.remove('hidden');
      if (classroomBadge) classroomBadge.classList.add('hidden');
      openModal(crudModal);
    }
    function openEdit(tr) {
      const d = tr.dataset;
      crudTitle.textContent = 'Edit Agenda';
      ...
      // Auto-group section: if classroom already linked, disable checkbox and show badge
      if (d.agClassroomId) {
        if (inputAutoGroup) { inputAutoGroup.checked = true; inputAutoGroup.disabled = true; }
        if (classroomBadgeId) classroomBadgeId.textContent = d.agClassroomId;
        if (classroomBadge)  classroomBadge.classList.remove('hidden');
      } else {
        if (inputAutoGroup) { inputAutoGroup.checked = false; inputAutoGroup.disabled = false; }
        if (classroomBadge) classroomBadge.classList.add('hidden');
      }
    ...
    document.getElementById('agendaOpenCreate')?.addEventListener('click', openCreate);
    document.getElementById('agendaCrudCancel')?.addEventListener('click', () => closeModal(crudModal));
    document.getElementById('agendaModalBackdrop')?.addEventListener('click', () => closeModal(crudModal));
    document.getElementById('agendaDeleteCancel')?.addEventListener('click', () => closeModal(deleteModal));
    document.getElementById('agendaDeleteBackdrop')?.addEventListener('click', () => closeModal(deleteModal));
